...
export default function CountElem({ id, item, sellerMe, accessToken, goodCount, isGood, page }: Props) {
    return (
        <section className={styles.countElem}>
            {["normal", "admin"].includes(page) && (
                <>
                <div className={styles.goodDiv}>
                    <Good id={id} sellerMe={sellerMe} accessToken={accessToken} initialCount={goodCount} initialGood={isGood} page={page as "normal" | "admin"} />
                </div>
