...
export function useLikeStatus(itemId: string) {
    return useSWR<{ isGood: boolean }>(`${process.env.NEXT_PUBLIC_API_URL}/item-like/${itemId}/status`, fetcher);
}

export function useLikeCount(itemId: string) {
    return useSWR<LikeCountResponse, Error>(`${process.env.NEXT_PUBLIC_API_URL}/item-like/${itemId}/count`, fetcher);
}
