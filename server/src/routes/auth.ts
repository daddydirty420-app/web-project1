...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      const { id, userName, admin, accessToken, refreshToken } =
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      const { id, email, userName, admin, accessToken, refreshToken } =
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      const resetRecord = await TokenPasswordReset.findOne({
      ...
      if (!user.email_verified) {
        ...
        res
          .status(403)
          .json({ message: 'このアカウントは有効ではありません。' });
        return;
      }
      ...
      if (!regex.test(password)) {
        res.status(400).json({
          message:
            'パスワードは8文字以上の半角英数字で、小文字と数字を含めてください。',
        });
        return;
      }
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      if (newEmail === user.email) {
        res
          .status(400)
          .json({ message: '現在と異なるメールアドレスを入力してください。' });
        return;
      }
      ...
      res.status(200).json({
        message:
          '新しいメールアドレスにメールを送信しました。メールアドレスの変更はまだ完了しておりません。',
      });
    } catch (err) {
      next(err);
    }
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
      ...
      if (!emailTokenData) {
        res
          .status(404)
          .json({ message: '新しいメールアドレスデータが見つかりません。' });
        return;
      }
