    ...
    try {
        const data = await ShopInfoEdit.findByPk(shopEditId, {
            ...
            include: [
                {
                    model: BankAccount,
                    attributes: ["id", "bank_name", "bank_code", "branch", "branch_code", "account_type_id", "account_number", "meigi"],
                    include: [
    ...
    try {
        const data = await ShopInfoEdit.findByPk(shopEditId, {
            include: [
                ...
                {
                    model: ShopInfo,
                    attributes: ["id", "company_name", "email", "phone_number", "homepage_url", "open_date_time", "company_number", "capital", "member_count"],
                    include: [
                        {
                            model: Address,
                            include: [
